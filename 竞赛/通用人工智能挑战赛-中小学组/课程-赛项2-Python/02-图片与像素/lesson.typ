...
- 本讲起需要 pillow：`python -m pip install pillow`（已换清华源则很快）
- 教师机验证：`python check_env.py`，PIL 一行显示 [OK] 即可开课
- 课堂脚本：本讲 `课堂脚本/step1` 至 `step5`，逐个运行；step1 可删掉行首 `#` 看弹窗图片
- 素材：`测试素材/orange / not_orange / not_orange_easy` 各 40 张
